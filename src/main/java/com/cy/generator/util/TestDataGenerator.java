	...
	public String getDBUnitTestData(String columnName,String javaType, int size) {
		...
		if(javaType.indexOf("String") >= 0) {
			if(size > columnName.length()) {
				...
				return columnName + com.cy.generator.util.StringHelper.randomNumeric(tempSize);
			}
			return com.cy.generator.util.StringHelper.randomNumeric(Math.min(size, MAX_SIZE));
		}
