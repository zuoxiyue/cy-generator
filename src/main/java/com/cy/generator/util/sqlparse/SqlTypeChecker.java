package com.cy.generator.util.sqlparse;
...
public class SqlTypeChecker {
	...
	public static boolean isSelectSql(String sourceSql) {
		return StringHelper.removeXMLCdataTag(com.cy.generator.util.sqlparse.SqlParseHelper.removeSqlComments(sourceSql)).trim().toLowerCase().matches("(?is)\\s*select\\s.*\\sfrom\\s+.*");
	}
	/**
	...
	public static boolean isUpdateSql(String sourceSql) {
		return StringHelper.removeXMLCdataTag(com.cy.generator.util.sqlparse.SqlParseHelper.removeSqlComments(sourceSql)).trim().toLowerCase().matches("(?is)\\s*update\\s+.*\\sset\\s.*");
	}
	/**
	...
	public static boolean isDeleteSql(String sourceSql) {
		String processedSql = StringHelper.removeXMLCdataTag(com.cy.generator.util.sqlparse.SqlParseHelper.removeSqlComments(sourceSql)).trim().toLowerCase();
