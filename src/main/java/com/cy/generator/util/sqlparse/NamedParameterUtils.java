...
public abstract class NamedParameterUtils {
	 ...
	 */
	public static com.cy.generator.util.sqlparse.ParsedSql parseSqlStatement(String sql) throws IllegalArgumentException {
		if(sql == null ) throw new IllegalArgumentException("SQL must not be null");

		Set<String> namedParameters = new HashSet<String>();
		com.cy.generator.util.sqlparse.ParsedSql parsedSql = new com.cy.generator.util.sqlparse.ParsedSql(sql);
		...
		while (i < statement.length) {
			...
			if (c == ':' || c == '&' || c == '#' || c == '$') {
				...
				if (j - i > 1) {
					...
					String removedPrefixAndSuffixParameter = removePrefixAndSuffix(c,parameter,sql); //add by cy
	...
	//add by cy,增加是否需要强制要有#后缀用于ibatis2
