...
public class ForeignKey implements java.io.Serializable{
	...
	protected com.cy.generator.provider.db.table.model.Table parentTable ;
	protected String   tableName;
	protected ListHashtable columns ;
	protected ListHashtable parentColumns;
	
	public ForeignKey(com.cy.generator.provider.db.table.model.Table aTable, String tblName) {
		super();
		parentTable = aTable;
		tableName   = tblName;
	...
	private void initRelationship() {
		firstRelation   = "";
		secondRelation  = "";
		com.cy.generator.provider.db.table.model.Table foreignTable = null;
		try {
			foreignTable = (com.cy.generator.provider.db.table.model.Table) TableFactory.getInstance().getTable(tableName);
		} catch (Exception e) {
			e.printStackTrace();
		}
		List parentPrimaryKeys    = parentTable.getPkColumns();
		List foreignPrimaryKeys   = foreignTable.getPkColumns();
	...
	private boolean hasAllPrimaryKeys(List pkeys, ListHashtable cols) {
		...
		for (int i=0;i<numKeys;i++) {
			com.cy.generator.provider.db.table.model.Column col = (com.cy.generator.provider.db.table.model.Column) pkeys.get(i);
	...
	public boolean isParentColumnsFromPrimaryKey() {
		boolean isFrom = true;
		List keys = parentTable.getPkColumns();
	...
	private boolean primaryKeyHasColumn(String aColumn) {
		...
		for (int i=0;i<numKeys;i++) {
			com.cy.generator.provider.db.table.model.Column sqlCol = (Column)parentTable.getPkColumns().get(i);
	...
	public com.cy.generator.provider.db.table.model.Table getSqlTable() {
		com.cy.generator.provider.db.table.model.Table table = null;
		try {
			table = (com.cy.generator.provider.db.table.model.Table) TableFactory.getInstance().getTable(tableName);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return table;
