...
<mapper namespace="${basepackage}.${namespace}.mapper.${className}Mapper">
	...
	<!-- 可根据自己的需求，是否要使用 -->
    <resultMap type="${basepackage}.${namespace}.entity.vo.${className}Vo" id="BasicsResultMap">
	...
	<insert id="insert" useGeneratedKeys="true" keyProperty="${table.pkColumn.columnNameFirstLower}" parameterType="${basepackage}.${namespace}.entity.${className}">
	...
	<!-- 批量新增 -->
	<insert id="insertBatch" parameterType="${basepackage}.${namespace}.entity.${className}">
		INSERT INTO ${table.sqlName} (
		<#list table.columns as column> ${column.sqlName}<#if column_has_next>,</#if></#list>
		) VALUES
	...
	<!-- 查询列表 -->
	<select id="queryList"  resultMap="BasicsResultMap" parameterType="${basepackage}.${namespace}.entity.${className}">
	...
	<!-- 查询总记录数 -->
 	<select id="queryTotal" resultType="int" parameterType="${basepackage}.${namespace}.entity.${className}">
